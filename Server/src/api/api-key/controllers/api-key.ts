...
export default factories.createCoreController('api::api-key.api-key', ({ strapi }: { strapi: Core.Strapi }) => ({
  /**
  ...
  async create(ctx) {
    ...
    if (existingKey) {
      return ctx.badRequest(`You already have an API key for ${provider}. Delete it first or update it.`);
    }
